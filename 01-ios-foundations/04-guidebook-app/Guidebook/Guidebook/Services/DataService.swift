...
struct DataService {
    /// Retrieves a city for SwiftUI preview.
    static func fetchPreviewCity() -> City {
            City(name: "Tokyo",
    ...
    /// Retrieves an array of cities.
    func fetchCities() -> [City] {
        // Get the file path to 'DemoData.json'
        guard let filePathURL = Bundle.main.url(forResource: "DemoData", withExtension: "json") else {
            // file not found, so return empty array
            return []
        }
        
        do {
            // Read the contents of the file and convert into Data
            let data = try Data(contentsOf: filePathURL)
            
            // Decode Data into [City] objects
            let cities = try JSONDecoder().decode([City].self, from: data)
            return cities
        } catch {
            print("Couldn't read the file or the data was not in correct format.")
        }
