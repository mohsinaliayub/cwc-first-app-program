# Jokes App
A simple challenge to test your understanding of the basics of networking, connecting to an API and retrieving data from it. 

## Setup
- API: https://jokeapi.dev/
- Authentication: None
- Endpoint: https://v2.jokeapi.dev/joke/Any?type=single

## Challenge
- Build a UI that lets you scroll through a list of movie posters
- Each movie poster should have a label under it
- Background of the app should be black
- Create a "DataService" struct to serve the data (just like we did with the SushiMenu App)

- Build a UI that has a Text label in the center of the screen. At the bottom, put a button.
- When the button is tapped, it sends the API request
- When the response returns, parse the data and display the joke in the Text label
- You should be able to keep tapping the button to show a different joke in the Text label
- Make sure you use "async" and "await".


## Goal
Fetch and display a random joke when user taps the button.


## Solution
Solution can be found [here](https://www.dropbox.com/scl/fi/xopy0eu436bplb2w4cp0u/Networking-Module-1-Challenge-1-Project.zip?rlkey=9ggl7pudef796v3sik5qu7id4&dl=1).

You can check the solution and this app code to see if they do the exact same thing.

# Screenshots
<img src="Screenshots/App.png" width="231" height="500" alt="Jokes App Home View">
